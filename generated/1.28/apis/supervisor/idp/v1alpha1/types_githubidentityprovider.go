...
type GitHubAPIConfig struct {
	...
	// +kubebuilder:validation:XValidation:message="Do not specify a URL, only a domain name",rule="!isURL(self)"
	// +kubebuilder:validation:XValidation:message="Do not specify a path, only a domain name",rule="!isURL(self) && isURL('https://'+self) && url('https://'+self).getEscapedPath() == ''"
	// +kubebuilder:validation:XValidation:message="Do not specify a query, only a domain name",rule="!isURL(self) && isURL('https://'+self) && size(url('https://'+self).getQuery()) < 1"
	// +kubebuilder:validation:XValidation:message="Do not specify a fragment, only a domain name",rule="!self.contains('#')"
	// +optional
...
const (
	// GitHubUseTeamNameForGroupName specifies using the GitHub team's `name` attribute as the downstream group name.
...
type GitHubIdentityProviderSpec struct {
	...
	// OrganizationLoginPolicy must be set to "AllOrganizationsAllowed" if allowedOrganizations is empty.
	//
	...
	// +optional
	OrganizationLoginPolicy *GitHubOrganizationLoginPolicy `json:"organizationLoginPolicy"`
...
type GitHubIdentityProvider struct {
	...
	// +kubebuilder:validation:XValidation:message="organizationLoginPolicy must be 'AllowedOrganizationsOnly' if allowedOrganizations are listed, or 'AllOrganizationsAllowed' if no allowedOrganizations are listed",rule="(self.organizationLoginPolicy == 'AllowedOrganizationsOnly' && has(self.allowedOrganizations) && size(self.allowedOrganizations) > 0) || (self.organizationLoginPolicy == 'AllOrganizationsAllowed' && (!has(self.allowedOrganizations) || size(self.allowedOrganizations) < 1))"
