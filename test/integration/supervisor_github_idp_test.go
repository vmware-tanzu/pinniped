...
func TestGitHubIDPStaticValidationOnCreate_Parallel(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "spec.gitHubAPI.tls.certificateAuthorityData must be a PEM bundle starting with '-----BEGIN CERTIFICATE----' and then base64-encoded",
			inputSpec: idpv1alpha1.GitHubIdentityProviderSpec{
				AllowAuthentication: idpv1alpha1.GitHubAllowAuthenticationSpec{
					Organizations: idpv1alpha1.GitHubOrganizationsSpec{
						Policy: ptr.To(idpv1alpha1.GitHubAllowedAuthOrganizationsPolicyAllGitHubUsers),
...
func TestGitHubIDPInvalidCertificateAuthorityDataStaticValidationOnCreate_Parallel(t *testing.T) {
	adminClient := testlib.NewKubernetesClientset(t)
	skipCELTests := !testutil.KubeServerMinorVersionAtLeastInclusive(t, adminClient.Discovery(), 26)
	...
	tests := []struct {
		name              string
		caBundleUnencoded string
	}{
		{
			name:              "empty bundle not allowed",
			caBundleUnencoded: "",
		},
		{
			name:              "invalid certificate not allowed",
			caBundleUnencoded: "foo",
		},
		{
			name:              "public keys not allowed",
			caBundleUnencoded: "-----BEGIN PUBLIC KEY-----",
		},
		{
			name:              "must begin with exactly '-----BEGIN CERTIFICATE-----'", // ends with 5 -----
			caBundleUnencoded: "-----BEGIN CERTIFICATE----",                            // ends with 4 ----
		},
	}
	for _, tt := range tests {
		tt := tt
		t.Run(tt.name, func(t *testing.T) {
			...
			input := &idpv1alpha1.GitHubIdentityProvider{
				...
				Spec: idpv1alpha1.GitHubIdentityProviderSpec{
					GitHubAPI: idpv1alpha1.GitHubAPIConfig{
						TLS: &idpv1alpha1.TLSSpec{
							CertificateAuthorityData: base64.StdEncoding.EncodeToString([]byte(tt.caBundleUnencoded)),
						},
					},
					AllowAuthentication: idpv1alpha1.GitHubAllowAuthenticationSpec{
						Organizations: idpv1alpha1.GitHubOrganizationsSpec{
			...
			_, err := gitHubIDPClient.Create(ctx, input, metav1.CreateOptions{})
			require.ErrorContains(t, err, "spec.gitHubAPI.tls.certificateAuthorityData must be a PEM bundle starting with '-----BEGIN CERTIFICATE----' and then base64-encoded")
		})
	}
}
